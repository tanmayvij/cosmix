		...
		if(!empty($_FILES['csv_file']['name']) && in_array($_FILES['csv_file']['type'],$csv_types))
		{
			if(is_uploaded_file($_FILES['csv_file']['tmp_name']))
			{
				...
				while(($line = fgetcsv($file)) !== FALSE)
				{
					...
					{
						mysqli_query($link, "UPDATE `books` SET `book_name`='$line[1]',`username`=$uname,`status`=$line[3],`doi`=$doi,`dor`=$dor WHERE book_id = $line[0];");
					}
					else
					{
						
						mysqli_query($link, "INSERT INTO `books` VALUES($line[0], '$line[1]', $uname, $line[3], $doi, $dor);");
					}
					if(empty(mysqli_error($link)))
						$ctr++;
					else
					{
						die("Unexpected error occurred. Please try again. " . $ctr . " Records successfully imported.<br><a href='csv.php?type=import'>Back</a>");
					}
				}
				fclose($file);
